...
import { confirmNetwork, setRequest, setupNetwork } from '../utils/api';
    ...
    store.getMany(Event).subscribe((events) => {
      try {
        ...
        if (events.data instanceof Array) {
          ...
          expect((data.relationships?.['images'].data as Array<IDefinition>)[0]).toEqual({
            type: 'image',
            id: '1',
          });
    ...
    store
      ...
      .pipe(
        switchMap((ev) => {
          ...
          return events.next?.() as Observable<any>;
        }),
        switchMap((ev2: Response<IJsonapiModel>) => {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          return events2.prev?.() as Observable<any>;
        }),
