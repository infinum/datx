...
export interface IMockArgs extends Partial<IRequestOptions> {
  hostname?: string;
  query?: boolean | string | (() => boolean) | Record<string, unknown>;
}
...
function fetchInterceptor(
  ...
  let request$ = new Observable<IRawResponse>((subscriber) => {
    ...
    if (request.reqheaders) {
      expect(
        Object.assign({}, config.defaultFetchOptions?.['headers'], requestHeaders),
      ).toMatchObject(request.reqheaders);
    }

    if (request.data) {
      const expected =
        typeof request.data === 'string' ? request.data : JSON.stringify(request.data);
      const received = body && typeof body === 'string' ? body : JSON.stringify(body);
