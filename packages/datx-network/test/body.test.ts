...
import { body, setUrl, method, HttpMethod } from '../src';
...
describe('body', () => {
  ...
  it('should send something if the method is supported', async () => {
    ...
    expect(request['lastHeaders']).toEqual({ 'content-type': 'application/vnd.api+json' });
  });

  it('should send the default content type if body type is raw', async () => {
    const request = new MockBaseRequest('foobar').pipe(
      setUrl('foobar'),
